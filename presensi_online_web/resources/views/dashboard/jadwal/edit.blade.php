...
<div class="content">
    <nav class="breadcrumb bg-white push">
        ...
        <a href="{{route('dosen.jadwal.edit', ['id' => $jadwal->id_jadwal])}}" class="breadcrumb-item active">Edit Jadwal</a>
    </nav>
</div>
@endsection
@section('main')
    ...
    <div class="block">
       <div class="block-header block-header-default">
          <div class="row">
            ...
            <div class="font-size-lg font-w600">&emsp;Edit Jadwal</div>
          </div>
       </div>
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="form-group">
                            ...
                            <input value="{{$jadwal->tahun_ajaran}}" required autocomplete="off" type="text" class="form-control" id="input-name" name="tahun_ajaran">
                        </div>
                    </div>
                </div>
                <div class="row">
                    <div class="form-group col-lg-6">
                        ...
                        <input autocomplete="off" type="time" class="js-flatpickr form-control bg-white"
                            ...
                            data-time_24hr="true" required value="{!! substr($jadwal->jam_mulai,0,5) !!}">
                    </div>
                    <div class="form-group col-lg-6">
                        ...
                        <input autocomplete="off" type="time" class="js-flatpickr form-control bg-white"
                            ...
                            data-enable-time="true" data-no-calendar="true" data-date-format="H:i"
                            data-time_24hr="true" required value="{!! substr($jadwal->jam_akhir,0,5) !!}">
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <input value="{{ $jadwal->semester }}" required autocomplete="off" type="text" class="form-control" id="input-name" name="semester">
                        </div>
                    </div>
                            ...
                            <select class="js-select2 form-control" id="example-select2"
                                name="kelas" style="width: 100%;" data-placeholder="pilih kelas" required>
                                    @foreach ($kelass as $kelas)
                                        <option <?php if($jadwal->id_kelas == $kelas->id) echo 'selected'; ?> value="{{$kelas->id}}">{{$kelas->nama_kelas}} - {{$kelas->lokasi_kelas}}</option>
                                    @endforeach
                            </select>
                        </div>
                    </div>
                            ...
                            <select class="js-select2 form-control" id="example-select2"
                                name="matakuliah" style="width: 100%;" data-placeholder="pilih matakuliah" required>
                                    @foreach ($matakuliahs as $matakuliah)
                                        <option <?php if($jadwal->id_matakuliah == $matakuliah->id) echo 'selected'; ?> value="{{$matakuliah->id}}">{{$matakuliah->nama_matakuliah}}</option>
                                    @endforeach
                            </select>
                        </div>
                    </div>
                        ...
                        <select class="js-select2 form-control" id="example-select2"
                            name="dosen" style="width: 100%;" data-placeholder="pilih dosen" required>
                                @foreach ($dosens as $dosen)
                                    <option <?php if($jadwal->id_dosen == $dosen->nip) echo 'selected'; ?> value="{{$dosen->nip}}">{{$dosen->nama_dosen}} - {{$dosen->departement}}</option>
                                @endforeach
                        </select>
                    </div>
                </div>
